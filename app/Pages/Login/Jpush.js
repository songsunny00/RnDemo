/*
 * @Author: badx.龙腾归海 
 * @Date: 2018-07-12 13:28:06 
 * @Last Modified by: badx.龙腾归海
 * @Last Modified time: 2018-08-05 00:42:53
 * @Description: 极光推送例子
 */
...
export default class App extends Component {
    ...
    jumpSecondActivity() {
        console.warn('jump to SecondActivity')
    ...
    onStopPress() {
        ...
        console.warn('Stop push press')
    }

    onResumePress() {
        JPushModule.resumePush()
        console.warn('Resume push press')
    }
    ...
    setTag() {
        if (this.state.tag) {
            ...
            JPushModule.setTags(this.state.tag.split(','), map => {
                if (map.errorCode === 0) {
                    console.warn('Tag operate succeed, tags: ' + map.tags)
                } else {
                    console.warn('error code: ' + map.errorCode)
                }
            })
        }
    }
    ...
    addTag = () => {
        console.warn('Adding tag: ' + this.state.tag)
        JPushModule.addTags(this.state.tag.split(','), map => {
            if (map.errorCode === 0) {
                console.warn('Add tags succeed, tags: ' + map.tags)
            } else {
                console.warn('Add tags failed, error code: ' + map.errorCode)
            }
        })
    }

    deleteTags = () => {
        console.warn('Deleting tag: ' + this.state.tag)
        JPushModule.deleteTags(this.state.tag.split(','), map => {
            if (map.errorCode === 0) {
                console.warn('Delete tags succeed, tags: ' + map.tags)
            } else {
                console.warn('Delete tags failed, error code: ' + map.errorCode)
            }
        })
    }

    checkTag = () => {
        console.warn('Checking tag bind state, tag: ' + this.state.tag)
    ...
    getAllTags = () => {
        JPushModule.getAllTags(map => {
            if (map.errorCode === 0) {
                console.warn('Get all tags succeed, tags: ' + map.tags)
            } else {
                console.warn('Get all tags failed, errorCode: ' + map.errorCode)
            }
        })
    }
    ...
    cleanAllTags = () => {
        JPushModule.cleanTags(map => {
            if (map.errorCode === 0) {
                console.warn('Clean all tags succeed')
            } else {
                console.warn('Clean all tags failed, errorCode: ' + map.errorCode)
            }
        })
    }
    ...
    setAlias() {
        if (this.state.alias !== undefined) {
            JPushModule.setAlias(this.state.alias, map => {
                console.warn(map);
                if (map.errorCode === 0) {
                    console.warn('set alias succeed')
                } else {
    ...
    deleteAlias = () => {
        console.warn('Deleting alias')
        JPushModule.deleteAlias(map => {
            console.warn(map);
            if (map.errorCode === 0) {
                console.warn('delete alias succeed')
            } else {
                console.warn('delete alias failed, errorCode: ' + map.errorCode)
            }
        })
    }
    ...
    getAlias = () => {
        JPushModule.getAlias(map => {
            console.warn(map);
            if (map.errorCode === 0) {
                console.warn('Get alias succeed, alias: ' + map.alias)
            } else {
                console.warn('Get alias failed, errorCode: ' + map.errorCode)
            }
        })
    }
    ...
    componentDidMount() {
        ...
        JPushModule.addReceiveNotificationListener(map => {
            ...
            // console.warn(extra.key + ": " + extra.value);
        })
        ...
        //         key2: 'value2'
        //     },
        //     fireTime: currentDate.getTime() + 2000,
        //     badge: 8,
        //     sound: 'fasdfa',
        //     subtitle: 'subtitle',
        // }
